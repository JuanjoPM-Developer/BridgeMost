...
def generate_config(
    ...
    lines = [
        ...
        f"# Generated by teleghost setup",
        "",
        "telegram:",
...
""")
    ...
    pat = await create_pat(mm_url, session["token"], mm_user_id, "teleghost-bridge")
    if not pat:
...
╚══════════════════════════════════════╝{N}
  ...
  Start TeleGhost:
    {G}python3 -m teleghost{N}

  Or with systemd:
    {G}sudo systemctl start teleghost{N}
...
def main():
    """Entry point for `teleghost setup` or `python -m teleghost.setup`."""
